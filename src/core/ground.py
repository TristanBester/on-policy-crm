...
class LetterWorld(gym.Env):
    ...
    def __init__(self, switch_proba: float = 0.5) -> None:
        """Initialize the environment.

        Args:
            switch_proba (float): Probability of switching symbol A to B when seen.
        """
        super().__init__()
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if not self.symbol_seen and np.array_equal(
            ...
            self.symbol_seen = np.random.random() < self.switch_proba
